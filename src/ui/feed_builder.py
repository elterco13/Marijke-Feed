...
def _render_profile_form(profile: Profile | None) -> None:
    ...
    with st.form(key=f"profile_form_{form_id}"):
        col1, col2 = st.columns(2)
        with col1:
            name = st.text_input("Feed Name *", value=defaults["name"])
        with col2:
        ...
        # We need a dummy object to preview the query during render, although Streamlit forms don't update live.
        # But we can at least show the ontology options cleanly.
        ...
        selected_exc = []
        with c4:
            st.markdown("#### Exclusions (Noise Filters)")
        ...
        st.markdown("### ⚙️ Settings & Overrides")
